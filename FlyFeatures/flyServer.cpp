...
uint16_t CFlyServerConfig::g_max_ddos_connect_to_me = 10; // Íå áîëåå 10 êîííåêòîâ íà îäèí IP â òå÷åíèè ìèíóòû
uint16_t CFlyServerConfig::g_ban_ddos_connect_to_me = 10; // Áëîêèðóåì ïîäêëþ÷åíèÿ ê ýòîìó IP â òå÷åíèè 10 ìèíóò

uint16_t CFlyServerConfig::g_interval_flood_command = 1;  // Ñêîëüêî ñåêóíä àãðåãèðóåì îäèíàêîâûå êîìàíäû
uint16_t CFlyServerConfig::g_max_flood_command = 100;       // Íå áîëåå 100 îäèíàêîâûõ êîìàíä â ñåêóíäó
uint16_t CFlyServerConfig::g_ban_flood_command = 10;      // Áëîêèðóåì íà 10 ñåêóíä êîìàíäû åñëè ïîïàëè â áàí
uint16_t CFlyServerConfig::g_unique_files_for_virus_detect = 2;
bool     CFlyServerConfig::g_is_append_cid_error_log = true; // Äîáàâëÿòü ID ê ëîãó îøèáîê
...
uint16_t CFlyServerConfig::g_max_unique_tth_search  = 10; // Íå ïðèíèìàåì â òå÷åíèè 10 ñåêóíä îäèíàêîâûõ ïîèñêîâ ïî TTH äëÿ îäíîãî è òîãî-æå öåëåâîãî IP:PORT (UDP)
uint16_t CFlyServerConfig::g_max_unique_file_search = 10; // Íå ïðèíèìàåì â òå÷åíèè 10 ñåêóíä îäèíàêîâûõ ïîèñêîâ ïî File äëÿ îäíîãî è òîãî-æå öåëåâîãî IP:PORT (UDP)
...
const std::vector<CServerItem> CFlyServerConfig::getMirrorTestPortServerArray()
{
	...
	{
		// Åñëè îïðåäåëåí ëîêàëüíûé òåñò-ñåðâåð - èñïîëüçóåì íà íåãî
...
{
	// TODO - óáðàòü ëèøíèå ñòðîêè èç ðåçóëüòàòà
		...
		do
		{
			...
			if (l_end_line != string::npos)
			{
				...
				for (auto i = g_exclude_tag_inform.cbegin(); i != g_exclude_tag_inform.cend(); ++i)
				{
					...
					if (l_tag_begin == 0  // Òýã â íà÷àëå ñòðîêè?
					        && l_cur_line.size() > l_end_index // Ïîñëå íåãî åñòü ìåñòî?
					        && (l_cur_line[l_end_index] == ':' || l_cur_line[l_end_index] == ' ') // Ïîñëå òýãà ïðîáåë èëè ':'
					   )
					{
						l_ignore_tag = true;
						break;
...
#else
		const string l_url_config_file = "http://etc.dchub.net/etc/flylinkdc-config-r6xx.xml";
#endif
		...
		try
		{
			...
			if (l_xml.findChild("Bootstraps"))
			{
				...
				if (l_xml.findChild("fly-server"))
				{
...
#ifdef FLYLINKDC_USE_GATHER_STATISTICS
					// Ïðåäóñìàòðèâàåì àëüòåðíàòèâíûé ñåðâåð ñòàòèñòèêè
					...
					m_min_file_size = Util::toInt64(l_xml.getChildAttrib("min_file_size")); // Â êîíôèãå min_size - ïåðåèìåíîâàòü
					dcassert(m_min_file_size);
					//m_max_size_value = Util::toInt(l_xml.getChildAttrib("max_size_value")); // Íå èñïîëüçóåòñÿ íî åñòü â êîíôèãå - óáðàòü âåçäå
	...
	try
	{
		...
		while (l_count_mirror < 2)
		{
			...
			if (l_magnet_result.empty())
			{
				...
				return false; // Êîí÷èëèñü ñòðàíè÷êè ïîèñêà
			}
#ifdef _DEBUG
			//LogManager::message("l_magnet_result = " + l_magnet_result);
#endif
			l_count_mirror = 2; // TODO - çàáèðàòü ÷èñëî çåðêàë èç lua
			try
			{
				Json::Value l_root;
				Json::Reader l_reader(Json::Features::strictMode());
...
bool CFlyServerConfig::isBlockShareExt(const string& p_name, const string& p_ext)
{
	...
	if (isCheckName(g_block_share_ext, p_ext)) // Êîíòðîëü ïî ðàñøèðåíèÿì
	{
		return true;
	}
	else if (isCheckName(g_block_share_name, p_name)) // Êîíòðîëü ïî ôàéëàì íà òî÷íîå ñîîòâåñòâèå
	{
		return true;
	}
	else if (Wildcard::patternMatchLowerCase(p_name, g_block_share_mask)) // Ïîïàë ïîä ìàñêó ïëîõóþ
	{
		return true;
	}
	return false; // Ôàéë õîðîøèé
}
...
void CFlyServerAdapter::waitForFlyServerStop()
{
    dcdrun(m_debugWaits = true;)
        if (m_query_thread)
        {
            m_query_thread->join(m_dwMilliseconds); // Äîæäåìñÿ çàâåðøåíèÿ íî íå áîëåå 10 ñåê
        }
}
//===================================================================================================================================
...
{
#ifdef _DEBUG
	// TODO - ýìóëèðóåì òîðìîçíîé èíåò ::Sleep(10000);
#endif
    ...
    if (!ClientManager::isBeforeShutdown() && ::IsWindow(p_hMediaWnd) && !g_GetFlyServerArray.empty())
	{
		...
		// TODO - ñîõðàíèòü m_GetFlyServerArray â äðóãîì ìåñòå?
		dcassert(::IsWindow(p_hMediaWnd));
		dcassert(!ClientManager::isBeforeShutdown());
    ...
    CFlyServerAdapter::push_mediainfo_to_fly_server(); // Ñáðîñèì íà ôëàé-ñåðâåð íîâóþ ìåäèàèíôó, êîòîðóþ íàøëè ó ñåáÿ
}
//===================================================================================================================================
...
void CFlyServerAdapter::prepare_mediainfo_to_fly_serverL()
{
	...
	for (auto i = l_tth_map.begin(); i != l_tth_map.end(); ++i)
	{
		...
		{
			bool l_is_send_info = l_media_info.isMedia() && g_fly_server_config.isFullMediainfo() == false; // Åñòü ìåäèàèíôà è ñåðâåð íå æäåò ïîëíûé êîìïëåêò?
			if (g_fly_server_config.isFullMediainfo()) // Åñëè ñåðâåð æäåò îò íàñ òîëüêî ïîëíûé êîìïëåêò - ïðîâåðèì íàëè÷èå àòðèáóòíîé ñîñòàâÿëþùåé
				l_is_send_info = l_media_info.isMediaAttrExists();
			if (l_is_send_info)
			{
				CFlyServerKey l_info(i->first, i->second);
				l_info.m_media = l_media_info; // Ïîëó÷èëè ìåäèàèíôîðìàöèþ ñ ëîêàëüíîé áàçû
                {
                    CFlyLock(g_cs_set_array_fly_server);
...
static void getDiskAndMemoryStat(Json::Value& p_info)
{
	...
	p_info["CPUCount"] = CompatibilityManager::getProcessorsCount();
	{
		...
		// TODO - ñäåëàòü îáîõîä îáùåãî ìàññèâà
	...
	dcassert(l_mem_ok);
	if (l_mem_ok) // Ïîä Wine ìîæåò íå ðàáîòàòü
	{
	...
	Json::Value& l_handle_info = p_info["Handle"];
	{
		...
		dcassert(l_hc_ok);
		l_handle_info["Handle"] = int(l_handle_count); // TODO íàó÷èòü jsoncpp ïîíèìàòü DWORD
...
# define GR_USEROBJECTS_PEAK 4       /* Peak count of USER objects */
	...
	// TODO - Ñâîáîäíîå ìåñòî íà äèñêå ñèñòåìíîì è òîì, ãäå ñòîèò ôëàé-áàçà
	{
...
bool CFlyServerJSON::isTestPortOK(unsigned short p_port, const std::string& p_type, bool p_is_assert /*= false */)
{
	...
	if (g_DisableTestPort == false)
	{
		...
		if (p_is_assert)
		{
			...
			{
				g_is_first = true;
				// TODO - ìíîãî ñïàìà - ðàçîáðàòüñÿ ïî÷åìó.
    ...
    const string& p_name_test)
{
	...
	bool l_is_error = false;
	p_external_ip.clear();
	// Ðåãèñòðèðóåì ôàêò íà÷àëà çàïðîñà íà òåñò ïîðòîâ
	...
	// TODO - ïðèäåëàòü ñ÷åò÷èê òàéìàóòà è ïåðåäàâàòü åãî â ñòàòèñòèêó èëè â ñëåä ïàêåò?
	if (!l_is_send)
	{
		l_log.step("Error POST query");
		// TODO - ïî÷èñòèòü g_test_port_map ?
	}
	else
	{
		if (!l_result.empty())
	...
	if (!p_error.empty())
	{
		...
		if (CFlyServerConfig::isErrorLog(p_error_code))
		{
			...
			if (p_error != g_last_error_string)
			{
				...
				if (!l_is_send)
				{
					// TODO Ïåðåäà÷à íå óäàëàñü - ñêèíåì äàííûå â ôàéëû
				}
...
bool CFlyServerJSON::pushStatistic(const bool p_is_sync_run)
{
	...
	if (CFlyServerConfig::g_is_use_statistics)
	{
		...
		l_log.m_skip_stop = true;
		Json::Value  l_info;
		if (p_is_sync_run == false && l_is_flush_error == false) // Ïðè îñòàíîâå íå äåëàåì ýòîãî + åñëè áûëà îøèáêà ëîãèíà - òîæå ñêèïàåì
		{
			// Ñáðîñèì 50 çàïèñåé îòëîæåííîé ñòàòèñòèêè åñëè íàêîïèëàñü
			CFlylinkDBManager::getInstance()->flush_lost_json_statistic(l_is_flush_error);
		}
		else
		{
			l_info["IsShutdown"] = "1"; // Ïîñòàâèì ìàðêåð îñòàíîâà ôëàÿ
		}
		...
		// Àãðåãàöèîííûå ïàðàìåòðû
		{
			...
			// TODO - ýòè ïàðàìåòðû ìîæíî ïîñ÷èòàòü èç ìàññèâà Clients
...
#endif
			{
				...
				Json::Value l_error_info;
				{
					...
					auto appendError = [&l_error_info](const wchar_t * p_reg_value, const char * p_json_key)
					{
						...
						{
							Util::deleteRegistryValue(p_reg_value); // TODO - óäàëÿòü ñðàçó ïîñëå ÷òåíèÿ ïîêà îòêðûò êëþ÷èê
				...
				// Çàáåðåì ïðåäûäóùèå ìàðêåðû çàâåðøåíèÿ
				try
				{
		...
		bool l_is_send = false;
		bool l_is_error = false;
		if (l_is_flush_error == false) // Åñëè íå óäàëîñü ñáðîñèòü ïåðâûé ðàç - íåò èíåòà.
		{
		...
		if (!l_is_send || p_is_sync_run)
			// Åñëè íå óäàëîñü îòïðàâèòü èëè îòêëþ÷åíî/îòëîæåíî.
			// ñîáèðàåì ñòàòó ëîêàëüíî (÷òîáû â áóäóùåì ïîñòðîèòü àíàëèòèêó íà êëèåíòå)
		{
                                 ...
                                 const CServerItem* p_server /*= nullptr */)
{
	...
	{
		l_Server.setIp(g_debug_fly_server_url); // Ïåðåêðûâàåì àäðåñ ôëàé-ñåðâåðà äëÿ âñåõ ñåðâèñîâ íà îòëàäî÷íûé
	}
	...
	string l_log_string;
	if (g_fly_server_config.getZlibCompressLevel() > Z_NO_COMPRESSION) // Âûïîëíÿåì ñæàòèå çàïðîñà?
	{
		if (p_is_disable_zlib_in == false)
		{
			...
			// Íà êëèåíòå ïîêà æìåì ïî ìàêñèìóìó - íàãðóçêà ìåëêàÿ.
			// åñëè äåêîìïðåññèÿ áóäåò ñàæàòü CPU ñåðâåðà ìîæåì îòêëþ÷èòü ÷åðåç êîíôèã (íî ìàëîâåðîÿòíî)
			if (l_zlib_result == Z_OK)
			{
				if (l_dest_length < p_body.length()) // ìåëêèå ïàêåòû ìîãóò ñòàòü áîëüøå - îòêàçûâàåìñÿ îò ñæàòèÿ.
				{
					l_post_compress_query.resize(l_dest_length); // TODO - íóæåí ëè ýòîò ðåëîêåéò?
		...
		if (hConnect)
		{
			...
			if (hRequest)
			{
				                     ...
				                     l_is_zlib ? l_post_compress_query.size() : p_body.size()))
				{
					...
					*/
#endif
					// TODO îáîáùèòü â óòèëèòíóþ ôóíêöèþ è ïîäìåíèòü â îáëàñòè DHT ñ äðóãèì ñòàðòîâûì êîýô-öåíòîì. äëÿ óìåíüøåíèÿ êîë-âà ðåëîêàöèé
					// TODO êîýô. ìîæíî âûñ÷èòûâàòü äèíàìè÷åñêèå è íå äåëàëàòü 10-êîé
					if (l_zlib_blob.size())
					{
						...
						{
							std::vector<unsigned char> l_decompress; // TODO ðàñøèðÿòü äèíàìè÷åñêè
	...
	if (!g_antivirus_counter.empty())
	{
		...
		for (auto i = l_copy_array.cbegin(); i != l_copy_array.cend(); ++i)
		{
			...
			for (auto j = i->second.cbegin(); j != i->second.cend(); ++j)
			{
				...
				if (j->m_hub_url.empty() && !j->m_hub_name.empty()) // Ïèøåì èìÿ åñëè àäðåñà íå îêàçàëîñü.
				...
				// Íå íóæåí l_file_array_item["count_file"] = j->m_count_file;
...
string CFlyServerJSON::connect(const CFlyServerKeyArray& p_fileInfoArray, bool p_is_fly_set_query, bool p_is_ext_info_for_single_file /* = false*/)
{
	...
	bool l_is_error = login(); // Çàïðîñèì ñâîé ÈÄ ó ñåðâåðà
	string l_result_query;
	if (l_is_error == false)
	{
		...
		if (!p_is_fly_set_query) // Äëÿ get-çàïðîñà âñå çàïèñè õîòÿò òîëüêî ðàñøèðåííóþ èíôó?
		{
			...
			if (l_count_only_counter == p_fileInfoArray.size())
			{
				...
				l_root["only_counter"] = 1; // Ïîìåñòèì ïðèçíàê â êîðåíü ÷òîáû íå ðàçìíîæàòü çàïðîñ âñåì.
			}
			...
			if (l_count_cache)
			{
				l_root["cache"] = l_count_cache;  // Äëÿ ñòàòèñòèêè TODO - ïîçæå óáðàòü
			}
			if (is_all_file_only_counter == false) // Åñëè íå ïðîñÿò òîëüêî ñ÷åò÷èêè - ïðîñòàâèì çàïðîñû íà ðàñøèðåííóþ èíôó
			{
				if (l_count_only_ext_info == p_fileInfoArray.size()) // Ïîëíóþ èíôó îòäàåì òîëüêî äëÿ òî÷å÷íûõ çàïðîñîâ
				{
					is_all_file_only_ext_info = true;
					l_root["only_ext_info"] = 1; // Ïîìåñòèì ïðèçíàê â êîðåíü ÷òîáû íå ðàçìíîæàòü çàïðîñ âñåì.
				}
				if (l_count_only_ext_info > 0 && l_count_only_ext_info != p_fileInfoArray.size())
				{
					l_root["different_ext_info"] = 1; // Ïîìåñòèì ïðèçíàê ðàçëè÷íûõ ñïîñîáîâ çàïðîñà ê ìåäèàèíôå
					// ÷òîáû íà ñòîðîíå ñåðâåðà íå äåëàòü ëèøíèé ïîèñê ïàðàìåòðà äëÿ êàæäîãî ôàéëà.
				}
			}
		}
		for (auto i = p_fileInfoArray.cbegin(); i != p_fileInfoArray.cend(); ++i)
		{
			// Íà÷èíàåì ñîçäàâàòü JSON ïàêåò äëÿ ïåðåäà÷è íà ñåðâåð.
			...
			// TODO - l_array_item["name"] = i->m_file_name;  èìÿ ïîêà íå ïîìíèòü. îíî ðàçíîå âñåãäà
			/* Ïðîáðîñ õèòîâ è âðåìåíè õåøà/ôàéëà òàê-æå èñêëþ÷àåì
			if(i->m_hit)
						...
						if (CFlyServerConfig::isSupportTag(j->m_param))
						{
							switch (j->m_stream_type)
							{
								...
								case MediaInfoLib::Stream_Audio:
								{
									...
									{
										// Îïðåäåëÿåì, â êàêóþ ñåêöèþ êàíàëà äîëæíà ïîïàñòü èíôîðìàöèÿ.
...
// string l_tmp;
		if (l_count > 0)  // Åñòü ÷òî ïåðåäàâàòü íà ñåðâåð?
		{
...
{
	// Ñîáåðåì îáùèé êîíòåíò äëÿ êàíàëîâ - TODO - ôèêñ êîïèïàñòû
...
string CFlyServerInfo::getMediaInfoAsText(const TTHValue& p_tth, int64_t p_file_size)
{
	...
	l_info.m_only_ext_info = true; // Çàïðîñèì ñ ñåðâåðà òîëüêî ðàñøèðåííóþ.
                            ...
                            bool p_compress_channel_attr)
{
	...
	{
		int l_count_audio_channel = 0;
		if (p_stream_type == MediaInfoLib::Stream_Audio  || p_stream_type ==  MediaInfoLib::Stream_Text) // ×èñëî êàíàëîâ ñ÷èòàåì òîëüêî äëÿ Audio è Text
		{
			...
			for (size_t j = 0; j < l_count; ++j)
			{
				...
				if (!l_value.empty())
				{
					...
					if (CFlyServerConfig::isSupportTag(l_str_param_name)) // TODO Fix fromT
					{
						...
						// Inform - Ñâîäíûé ïàðàìåòð - ðàñïàðñèì åãî äëÿ óäàëåíèÿ ëèøíèõ çàïèñåé è ïðîáåëîâ.
		...
		if (p_compress_channel_attr && l_count_audio_channel > 1) // Åñëè àóäèî-êàíàëîâ íåñêîëüêî. äîïîëíèòåëüíî îáðàáîòàåì ìàññèâ
		{
			// Óïàêóåì ìàññèâ êàíàëîâ åñëè èõ íåñêîëüêî è çíà÷åíèÿ ïðèçíàêîâ îäèíàêîâîå äëÿ âñåõ.
			// Ïîêà àëãîðèòì îáðàáîòêè äâóõ-ïðîõîäíûé
			// èçáûòî÷íûå àòðèáóòû äëÿ êàíàëîâ ïîìåòèì ôëàæêîì ÷òîáû íå ïèñàòü â áàçó.
			// TODO - ïðîòåñòèðîâàòü è ôèçè÷åñêîå óäàëåíèå ëèøíèõ çàïèñåé èç âåêòîðà
			std::map <std::string, std::pair< std::string, int> > l_channel_dup_filter;
			// Ïàðàìåòð MI - ïàðà - { çíà÷åíèå ïàðàìåòðà + ñ÷åò÷èê ðàçëè÷íûõ âàðèàíòîâ äëÿ çíà÷åíèé }
			for (auto j = p_media.m_ext_array.cbegin(); j != p_media.m_ext_array.cend(); ++j)
				// TODO [!] ýòîò ïðîõîä ìîæíî ïîìåñòèòü âíóòü îñíîâíîãî àëãîðèòìà çàïîëíåíèÿ.
				// âåäü íàì èçâåñòíî çàðàíåå ÷òî íà Audio - íåñêîëüêî êàíàëîâ áóäåò
			{
				if (j->m_stream_type == p_stream_type)
				{
					...
					{
						if (l_value.first.empty()) // Ïåðâûé ïàðàìåòð?
						{
			...
			// Ðàñ÷èòàëè ñòàòèñòèêó ïî àòðèáóòàì.
			// Îòìåòèì äóáëèêàòíûå çàïèñè ôëàæêîì äëÿ ïîñëåäóþùåãî èãíîðà ïðè çàïèñè â áàçó äàííûõ.
			for (auto k = p_media.m_ext_array.begin(); k != p_media.m_ext_array.end(); ++k)
			{
				if (k->m_stream_type == p_stream_type)
				{
					...
					if (l_channel_filter->second.second == 1) // Äëÿ âñåõ êàíàëîâ çíà÷åíèÿ ïàðàìåòðà îäèíàêîâîå?
					{
						k->m_channel  = CFlyMediaInfo::ExtItem::channel_all; // Êàíàë äëÿ ïåðâîãî âõîæäåíèÿ äåëàåì 255 (channel-All)
						l_channel_filter->second.second = -1; // Ñêèäûâàåì ñ÷åò÷èê â -1 ÷òîáû íà ñëåäóåùåì ïðîõîäå óäàëèòü çàïèñü
						continue;
					}
					if (l_channel_filter->second.second == -1) // Çíà÷åíèå ïàðàìåòðà óæå ïåðåíåñåíî â îáùèé êàíàë ñ êîäîì 255 íà ïðåäûäóùåé èòåððàöèè
						// à òåêóùàÿ çàïèñü èçáûòî÷íàÿ è åå ïèñàòü â áàçó íå íóæíî.
						// TODO - óäàëåíèå èç vector-à ïîêà íå äåëàåì. ïîçæå ýòà çàïèñü âñåðàâíî íå çàãðóçèòñÿ
					{
						k->m_is_delete = true;
					}
				}
...
bool getMediaInfo(const string& p_name, CFlyMediaInfo& p_media, int64_t p_size, const TTHValue& p_tth, bool p_force /* = false*/)
{
	...
	try
	{
		...
		if (l_media_result && !ClientManager::isBeforeShutdown())
		{
			...
			// Ëîêàëüíî ñîáèðàåì ìåäèàèíôó âñåãäà - ÷òîáû ïðîùå ðàñøèðÿòü ïîääåðæèâàåìûå ðàñøèðåíèÿ íà ôëàé ñåðâåðå â áóäóùåì.
			{
				// TODO - æåëàòåëüíî çâàòü ñî ñæàòèåì êàíàëîâ ïåðâûì (áóäåò áûñòðåå èç-çà ïîäñêàçêè p_compress_channel_attr)
				getExtMediaInfo(l_file_ext, p_size, g_media_info_lib, MediaInfoLib::Stream_Audio, p_media, true); // Ñîæìåì äóáëèêàòû â êàíàëàõ
...
//			getExtMediaInfo(l_file_ext, p_size, g_media_info_lib,MediaInfoLib::Stream_Image,p_media);
	...
	{
		// TODO ñþäà íå ïîïàäàåì åñëè SEH - íàéòè ñïîñîá è ïîïðàâèòü
		Util::deleteRegistryValue(FLYLINKDC_REGISTRY_MEDIAINFO_FREEZE_KEY);
