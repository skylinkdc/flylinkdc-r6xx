...
// Ïàðàìåòðû ïîäâåðæåííûå ïàäåíèþ - ïðîâåñòè äîïîëíèòåëüíóþ âàëèàäöèþ
...
//	setDefault(AUTO_COMPLETE_SEARCH, TRUE);
	...
	// TODO - ãðóçèòü ýòî èç ñåòè è îòëîæåííî êîãäà ïîíàäîáèòñÿ.
...
void SettingsManager::load(const string& aFileName)
{
	...
	catch (const SimpleXMLException&) // TODO Áèòûé êîíôèã XML https://crash-server.com/Problem.aspx?ProblemID=15638
	{
		dcassert(0);
	}
	...
	if (languageFile.empty() || l_version <= 7589 || !File::isExist(Util::getLocalisationPath() + languageFile))
	{
		...
		l_key_path += _T("FlylinkDC++");// _T(APPNAME); // Òóò íå ìåíÿåì èìÿ íà âèï
#ifdef _WIN64
	...
	//  set(PM_PASSWORD_HINT, STRING(DEF_PASSWORD_HINT));    //Æ¸ñòêî çàìåíèòü âñþ ñòðîêó.
#ifdef HOURLY_CHECK_UPDATE
	set(AUTOUPDATE_TIME, 24);
#endif // HOURLY_CHECK_UPDATE
	
	//óäàëèòü ÷åðåç íåñêîëüêî ðåëèçîâ
...
#undef REDUCE_LENGHT
	...
	if (l_auto) // Åñëè ïàðàìåòð èçìåíèëè â ìîìåíò çàãðóçêè - ñòàâèì ìàðêåð ÷òî íóæíî çàïèñàòüñÿ îáðàòíî â ôàéë.
	{
		isSet[key] = true;
	}
