...
void Colors::getUserColor(bool p_is_op, const UserPtr& user, COLORREF &fg, COLORREF &bg, unsigned short& p_flag_mask, const OnlineUserPtr& onlineUser)
{
	...
	if (p_is_op && onlineUser) // Âîçìîæíî ôèêñ https://crash-server.com/Problem.aspx?ClientID=guest&ProblemID=38000
	{
...
void FlagImage::init()
{
	m_flagImageCount = ResourceLoader::LoadImageList(IDR_FLAGS, m_images, 25, 16);
	dcassert(m_flagImageCount);
	dcassert(m_images.GetImageCount() <= 255); // ×òîáû íå ïðåâûñèòü 8 áèò
	
#ifdef FLYLINKDC_USE_CUSTOM_LOCATIONS
	if (!CompatibilityManager::isWine()) // ïîä ëèíóêñîì ïîêà ïàäàåì http://flylinkdc.blogspot.com/2010/08/customlocationsbmp-wine.html
	{
...
bool WinUtil::getUCParams(HWND parent, const UserCommand& uc, StringMap& sm) // TODO óáðàòü parent
{
...
bool WinUtil::checkCommand(tstring& cmd, tstring& param, tstring& message, tstring& status, tstring& local_message)
{
	...
	else if (stricmp(cmd.c_str(), _T("slots")) == 0)
	{
		...
		if (j > 0)
		{
			...
			ClientManager::infoUpdated(); // Íå çâàòü åñëè íå ìåíÿåòñÿ SLOTS_SET
		}
		else
		{
	...
	*/
	...
	// Â Êîíòàêòå. Îòêðûâàåì ñòðàíèöó ïî id.
	...
	// FILExt. To find a description of the file extension / Äëÿ ïîèñêà îïèñàíèÿ ðàñøèðåíèÿ ôàéëà.
	...
	else if (stricmp(cmd.c_str(), _T("blog")) == 0 || stricmp(cmd.c_str(), _T("b")) == 0)
	{
		...
		{
			WinUtil::openLink(_T("") + Text::toT(Util::encodeURI(Text::fromT(param))));
		}
	}
...
{
	p_icon_index = 0;
	auto x = Text::toLower(Util::getFileExtWithoutDot(aFileName)); //TODO ÷àñòî çîâåì
	if (x.compare(0, 3, "exe", 3) == 0)
	{
		// Ïðîâåðêà íà äâîéíûå ðàñøèðåíèÿ
		...
		if (dst_hdc)
		{
			...
			if (bmp)
			{
				...
				if (!supportsAlpha)
				{
					...
					::ExtTextOut(dst_hdc, 0, 0, ETO_OPAQUE, &rc, NULL, 0, NULL); // TODO - òóò âûâîäèòñÿ ïóñòîòà?
				}
#endif // FLYLINKDC_SUPPORT_WIN_XP
...
TCHAR WinUtil::CharTranscode(const TCHAR msg)
{
	...
	static const TCHAR Rus[] = L"¸éöóêåíãøùçõúôûâàïðîëäæýÿ÷ñìèòüáþ.¨!\"¹;%:?*()_+/ÉÖÓÊÅÍÃØÙÇÕÚÔÛÂÀÏÐÎËÄÆÝß×ÑÌÈÒÜÁÞ,";
...
void WinUtil::SetBBCodeForCEdit(CEdit& ctrlMessage, WORD wID)
{
	...
	if ((nEndSel - nStartSel) > 0) //×òî-òî âûäåëåíî, îáðàìëÿåì òýãîì, êóðñîð ñòàâèì â êîíöå
	{
		setString += startTag;
	...
	else    // Íè÷åãî íå âûáðàíî, ñòàâèì òýãè, êóðñîð ìåæäó íèìè
	{
		setString += startTag;
